    ...
    public class GeoService : IGeoService
    {
        ...
        public async Task<Result<object>> AddressByGeoAsyncV2(string? lngStr, string? latStr)
        {
            ...
            if (!ret.Any())
            {
                ...
                return Result<object>.Success(new RPBLAddressResultV3(), INIT_DATA_SUCCESS, "Đã khởi tạo xong dữ liệu", RunningParams.ProcessState.ToString());
            }

            var data = new List<RPBLAddressResultV3>();
                ret.ForEach(item => data.Add(new RPBLAddressResultV3(item)));
